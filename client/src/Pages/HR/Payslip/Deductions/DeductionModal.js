...
const DeductionModal = ({ onClose, onSave, editingDeduction }) => {
  ...
  useEffect(() => {
    if (editingDeduction) {
      setDeduction({
        title: editingDeduction.name, // Use 'name' from API
        description: editingDeduction.description,
        amount: editingDeduction.defaultAmount, // Use 'defaultAmount' from API
        includeAll: editingDeduction.appliesTo === "All", // Map appliesTo to includeAll
        includedEmployees: [],
        excludedEmployees: [],
      });
    }
  ...
  useEffect(() => {
    if (editingDeduction && employees.length > 0) {
      const included = editingDeduction.includedEmployees.map(id => 
        employees.find(e => e.id === id)
      ).filter(Boolean);
      const excluded = editingDeduction.excludedEmployees.map(id => 
        employees.find(e => e.id === id)
      ).filter(Boolean);
  ...
  const validateForm = () => {
    ...
    if (!deduction.includeAll && deduction.includedEmployees.length === 0) {
  ...
  const handleSaveClick = async () => {
    ...
    const deductionData = {
      ...
      defaultAmount: parseFloat(deduction.amount),
      appliesTo: deduction.includeAll ? "All" : "Specific",
      includedEmployees: deduction.includedEmployees.map(e => e.id),
      excludedEmployees: deduction.excludedEmployees.map(e => e.id),
    };
  
    
     onSave(deductionData);
     onClose();
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          ...
          <div className="mb-6">
            ...
            <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {deduction.includeAll
                    ? "Applies to all employees except excluded"
                    : "Applies only to selected employees"}
                </p>
              </div>
              <ToggleSwitch
                isOn={deduction.includeAll}
                handleToggle={() => setDeduction((prev) => ({
                  ...prev,
            ...
            {!deduction.includeAll && (
              <div className="p-4 border rounded-lg mt-4">
                ...
                >
                  {deduction.includedEmployees.length === 0 ? (
                    <p className="text-gray-400 text-center">Click to select employees</p>
                  ) : (
                    <div className="flex flex-wrap gap-2">
                      {deduction.includedEmployees.map((employee) => (
                        <EmployeeChip
                          key={employee.id}
                          employee={employee}
            ...
            {deduction.includeAll && (
              <div className="p-4 border rounded-lg mt-4">
                ...
                >
                  {deduction.excludedEmployees.length === 0 ? (
                    <p className="text-gray-400 text-center">Click to exclude employees</p>
                  ) : (
                    <div className="flex flex-wrap gap-2">
                      {deduction.excludedEmployees.map((employee) => (
                        <EmployeeChip
                          key={employee.id}
                          employee={employee}
      ...
      {isIncludeListOpen && (
        <EmployeeSelectionModal
          employees={employees}
          selectedIds={deduction.includedEmployees.map((e) => e.id)}
      ...
      {isExcludeListOpen && (
        <EmployeeSelectionModal
          employees={employees}
          selectedIds={deduction.excludedEmployees.map((e) => e.id)}
      ...
      {showToast && (
        <Toast
          ...
          message="The deduction has been successfully saved."
          duration={3000}
