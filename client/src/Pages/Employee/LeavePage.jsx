...
import LeaveStats from "../HR/Leave/LeaveStats";

import LeaveHistory from "../HR/Leave/LeaveHistory";
import LeaveDetails from "./LeaveDetails";
import RequestLeaveForm from "./LeaveRequestForm";
...
const LeaveManagement = () => {
  ...
  const fetchLeaveHistory = async (employeeId) => {
    try {
      const response = await fetch(
        `http://localhost:8000/getLeaves/${employeeId}`,
        {
          method: "GET",
          headers: {
  ...
  // Call function with specific employee ID
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-8">
      ...
      {selectedTab === "LeaveType" && (
        <LeaveType onClose={() => setShowDetailsModal(false)} />
      )}
      ...
      {showRequestForm && (
        <RequestLeaveForm onClose={() => setShowRequestForm()} />
      )}
    </div>
  );
};
